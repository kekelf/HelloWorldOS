...
public class Floppy {
	...
	//每一个盘面有80个磁道，也就是柱面(编号从0开始到79)
	private int CYLINDER_COUNT = 80; //80个柱面
	//每一个柱面有18个扇区（扇区的编号为1到18）
    ...
    /*
     * 最里面的ArrayList<byte[]>对应一个扇区
     * ArrayList<ArrayList<byte[]>> 对应一个磁道
     * HashMap<Integer,ArrayList<ArrayList<byte[]>> >对应一个盘面
     */
     ...
     * 
     * @param head 磁头
     * @param cylinder_num 柱面
     * @param sector_num 扇区
     * @param buf 数据数组
     * @function:将数据写入对应的位置，详细的过程就不看了
     */
